...
function ResultPanel({ side, isRo }: { side: SideState; isRo: boolean }) {
  ...
  return <div className="compare-result">
    ...
    <div className="compare-table"><table><thead><tr><th>#</th><th>{monthly ? (isRo ? 'Producător' : 'Producer') : (isRo ? 'Centru' : 'Center')}</th><th>{isRo ? 'Litri' : 'Liters'}</th>{monthly ? <><th>{isRo ? 'Tip lapte' : 'Milk type'}</th><th>G</th></> : <><th>{isRo ? 'Grăsime %' : 'Fat %'}</th><th>U.G.</th><th>{isRo ? 'Apă' : 'Water'}</th><th>{isRo ? 'Temp.' : 'Temp.'}</th><th>Aviz</th></>}</tr></thead><tbody>{rows.map((row: Record<string, any>, index: number) => <tr key={`${row.rowNumber}-${index}`}><td>{row.rowNumber}<small>{Math.round(Number(row.confidence || 0) * 100)}%</small></td><td>{valueText(monthly ? (row.producer || row.centerName) : row.collectionCenter)}</td><td>{valueText(row.liters)}</td>{monthly ? <><td>{valueText(row.milkType || data.milkType)}</td><td>{valueText(row.gValue)}</td></> : <><td>{valueText(row.fatPercent)}</td><td>{valueText(row.density)}</td><td>{valueText(row.water)}</td><td>{valueText(row.temperature)}</td><td>{valueText(row.noticeNumber)}</td></>}</tr>)}</tbody></table></div>
...
export function OcrComparisonScreen() {
  ...
  useEffect(() => { setImageRotation(0); if (!file) return setPreviewUrl(''); const url = URL.createObjectURL(file); setPreviewUrl(url); return () => URL.revokeObjectURL(url) }, [file])
  useEffect(() => {
  ...
  useEffect(() => { centerImagePreview(sourcePreviewRef.current) }, [imageRotation, zoom, previewUrl])
