...
First identify which of these three monthly document shapes is shown:
- detailed_multi_producer_grid: a wide handwritten daily grid with many producer names in the first column, day columns across the month, and final TOTAL / U.G. columns on the right. Return layoutType as "detailed".
- detailed_single_producer_daily_lines: one producer name is written at the top or side, followed by dated quantity entries underneath, sometimes with separate milk type sections such as VACA or BIVOL. Return layoutType as "detailed".
- overview_summary_grid: a printed summary grid with a collection center in the header and producer rows with liters, G. and U.G. columns. Return layoutType as "overview".
...
- For detailed_single_producer_daily_lines documents, return exactly one row for the producer using the final TOTAL as liters; do not return the individual dated entries as rows. Example: SITA - SERBAN followed by 03/07 242, 04/07 77, and TOTAL 2319 returns one row for SITA - SERBAN with liters 2319.
- For detailed_single_producer_daily_lines documents with separate milk type sections, return one row per milk type total for that producer; do not return the individual daily date rows. Example: a VACA total and a BIVOL total for the same producer returns two rows, one MILK-COW and one MILK-BUFF.
...
- The printed detailed layout can contain two right-side TOTAL / U.G. groups. Put the final far-right TOTAL L value into liters when it is visible. If only the nearer TOTAL L beside the day columns is legible, put that value into liters and warn that the far-right total was not confirmed.
...
- Mandatory check before returning JSON: independently trace every populated detailed-journal row horizontally into the right-side summary columns. Confirm that liters came from the final far-right TOTAL L when it is visible; otherwise use the nearer visible TOTAL L and warn that the far-right total was not confirmed. Then compare the returned row numbers with every visibly populated producer row and add any omitted rows. Do not claim completion until both checks have been performed.
